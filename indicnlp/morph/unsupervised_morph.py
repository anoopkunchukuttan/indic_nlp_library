...
class UnsupervisedMorphAnalyzer(MorphAnalyzerI):
    ...
    def __init__(self, lang, add_marker=False):
        ...
        self._script_range_pat = r"^[{}-{}]+$".format(
            chr(langinfo.SCRIPT_RANGES[lang][0]), chr(langinfo.SCRIPT_RANGES[lang][1])
        )
    ...
    def _contains_number(self, text):
        if self.lang in langinfo.SCRIPT_RANGES:
            for c in text:
                ...
                if (
                    offset >= langinfo.NUMERIC_OFFSET_START
                    and offset <= langinfo.NUMERIC_OFFSET_END
                ):
                    return True
        return False
    ...
    def morph_analyze(self, word):
        ...
        if self._morphanalysis_needed(word):
            ...
            if self.add_marker:
                m_list = [
                    "{}_S_".format(m) if i > 0 else "{}_R_".format(m)
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print(
            "Usage: python unsupervised_morph.py <infile> <outfile> <language> [<add_marker>]"
        )
        sys.exit(1)
